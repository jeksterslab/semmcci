...
intervals for the indirect effect in a simple mediation model where
variable `X` has an effect on variable `Y`, through a mediating variable
`M`. This mediating or indirect effect is a product of path coefficients
from the fitted model.

![The Simple Mediation
Model](https://raw.githubusercontent.com/jeksterslab/semmcci/main/.setup/latex/figures/png/simple.png)
...
In this example, confidence intervals for the standardized indirect
effect are generated by specifying the standardized indirect effect as a
derived parameter using the `:=` operator. The standardized indirect
effect in a simple mediation model involves paths $`a`$ and $`b`$, and
the standard deviations of $`X`$ and $`Y`$. It is given by
...
$`s_{e_{Y}}^{2}`$ and $`s_{e_{M}}^{2}`$ are the residual variances in
...
The row `indirect_std` corresponds to the confidence intervals for the
...
#> s2_X         1.0590 0.0472 20000 0.9675 1.1531
#> s2_em        0.7527 0.0339 20000 0.6865 0.8192
#> s2_ey        0.5462 0.0243 20000 0.4983 0.5936
#> indirect_std 0.2542 0.0173 20000 0.2210 0.2884
```
